...
void MakePlots(){
  TFile* f0 = new TFile("OneTon_geometry_XYplane_20180815.root","read");
  TCanvas*  c_geo = (TCanvas*)f0->Get("1ton_geo_xy");
  string filename = "Results_v12_allTrigCombinations_scanFiles9.txt.root";
...
/*
  // The hEndXY scatter plots for different triggers
  TCanvas* c[nbTrigs];
  TPaveText* pt[nbTrigs];
  TPad* p[nbTrigs];
  TH2F* h[nbTrigs];
  char name[120];
  for(int i=0; i<nbTrigs; i++){
    //c[i] = (TCanvas*)f0->Get("1ton_geo_xy");
    c[i] = (TCanvas*)c_geo->Clone();
    sprintf(name,"dir_%s/hEndXY_%s_zAtH4H5",strig[i].c_str(),strig[i].c_str()); // I have zAtH4H5 and z-500

    h[i] = (TH2F*)f->Get(name);
    sprintf(name,"Trig_%s",strig[i].c_str());
    h[i]->SetTitle(name);
    cout<<h[i]->GetTitle()<<endl;
    c[i]->Draw();
    //c[i]->DrawClone();
    c[i]->SetName(strig[i].c_str());
    c[i]->SetTitle(strig[i].c_str());
    h[i]->Draw("same colz");
    c[i]->Modified(); c[i]->Update();
    sprintf(name,"psd%d",i);
    p[i] = new TPad(name, name, 0., 0., 1, 1);
    p[i]->SetFillColor(0);
    p[i]->SetFillStyle(4000);
    p[i]->SetFrameFillStyle(0);
    p[i]->Draw();
    p[i]->cd();
    pt[i] = new TPaveText(0.65,0.1,0.85,0.2,"NB");
    sprintf(name,"TrigType_%s",strig[i].c_str());
    pt[i]->AddText(name);
    pt[i]->SetFillColor(0);
    //((TText*)pt[i]->GetListOfLines()->Last())->SetTextColor(kBlue);
    pt[i]->Draw();
    sprintf(name,"MC_Water_hEndXY_atZH4H5_trigType_%s.png",strig[i].c_str()); // zH4H5 or z-500
    c[i]->SaveAs(name);
  }
*/

  // npe distributions for different triggers
  gStyle->SetStatY(0.9);
  gStyle->SetStatX(0.9);
  gStyle->SetStatW(0.5);
  gStyle->SetStatH(0.4);  
  gStyle->SetOptStat(111110);
  TCanvas* cPMTNpe[nbTrigs];
  TH1F* hPMTNpe[nbTrigs][pmtnb];
  TH1F* hPMTNpe50ns[nbTrigs][pmtnb];
  char name[150];
  for(int i=0; i<nbTrigs; i++){
    sprintf(name,"cTrigType_%s",strig[i].c_str());
    cPMTNpe[i]=new TCanvas(name,name,1600,800);
    cPMTNpe[i]->Divide(4,2);
    //cout<<strig[i]<<endl;
    for(int j=0;j<pmtnb;j++){
      sprintf(name,"dir_%s/hCharge_2500ns_S%d_%sTrig",strig[i].c_str(),j,strig[i].c_str());
      hPMTNpe[i][j] = (TH1F*)f->Get(name);
      sprintf(name,"dir_%s/hCharge_50ns_S%d_%sTrig",strig[i].c_str(),j,strig[i].c_str());
      hPMTNpe50ns[i][j] = (TH1F*)f->Get(name);
      //cout<<"\tget pmt "<<j<<endl;
      cPMTNpe[i]->cd(j+1);
      gPad->SetLogy();
      hPMTNpe[i][j]->Draw(); hPMTNpe[i][j]->SetLineColor(1);
      hPMTNpe50ns[i][j]->Draw("sames"); hPMTNpe50ns[i][j]->SetLineColor(2); hPMTNpe50ns[i][j]->SetLineStyle(2);
      sprintf(name,"PMT_S%d_TrigType_%s",j,strig[i].c_str());
      hPMTNpe[i][j]->SetTitle(name);
      //hPMTNpe[i][j]->GetXaxis()->SetRangeUser(0,50);
      if(j==4 || j==5) hPMTNpe[i][j]->GetXaxis()->SetRangeUser(0,10);
    }
    sprintf(name,"npe_histograms_allStatistics/PMTnpe_%s_TrigType_%s.png",filename.substr(filename.length()-19,10),strig[i].c_str());
    cPMTNpe[i]->SaveAs(name);
    cPMTNpe[i]->Close();
  }
...
int main(){
    const int pmtnb = 8;
    const int nbTrigs = 27;
    const int nbOfMCDataPoints = 29; // in my example, I have 17 data points (17 runs under different attenuation parameters)
    const int file_index_as_data = 5; // maximmum is nbOfMCDataPoints-1
    ...
    string sMCFileNames[nbOfMCDataPoints]={
                        "Results_v12_allTrigCombinations_scanFiles1.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles2.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles3.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles4.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles5.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles9.txt.root", // used as data
                        "Results_v12_allTrigCombinations_scanFiles6.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles7.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles8.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles10.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles11.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles12.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles13.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles14.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles15.txt.root",
                        //"Results_v12_allTrigCombinations_scanFiles16.txt.root",
                        //"Results_v12_allTrigCombinations_scanFiles17.txt.root",
                        //"Results_v12_allTrigCombinations_scanFiles18.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles19.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles20.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles21.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles22.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles23.txt.root",
                        //"Results_v12_allTrigCombinations_scanFiles24.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles25.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles26.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles27.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles28.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles29.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles30.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles31.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles32.txt.root",
                        "Results_v12_allTrigCombinations_scanFiles33.txt.root"

                                          };
    double attenuationPar[nbOfMCDataPoints]={ // max. attenuation length in meters
                         202.158, // 1
                         99.3408, // 2
                         49.885,  // 3
                         30.8572, // 4
    ...
    */
    ...
    TFile* fcompare = new TFile("npe_dis_compare_test.root","recreate");
    gStyle->SetOptStat(0);
            ...
            for(int j=0;j<nbTrigs;j++){
                ...
                for(int k=0;k<nbin;k++){
                    ...
                    if(a>=10){
                        ...
                        if(a!=0 && b!=0){
                            hRes->SetBinContent( k+1+(nbin-(k+1))/2, b/a);
                            hRes->SetBinError(k+1+(nbin-(k+1))/2,b/a*TMath::Sqrt(1./b+1./a));
                ...
                hRes->GetYaxis()->SetTitle("Bin Entries Ratio: MC/Data");
                hRes->GetYaxis()->SetRangeUser(0,3);
                c_com->cd(3);
                gPad->SetPad(0,0.0,1,0.33);
                hChiSqDevi->Draw("p"); hChiSqDevi->SetMarkerStyle(8);
                hChiSqDevi->GetYaxis()->SetTitle("Chi2 value in bin, data - MC");
                c_com->Write();
                c_com->Close();
            }
        }
    ...
    for(int i=0; i<nbOfMCDataPoints; i++){ // looping through MC files
        ...
        for(int k=0; k<nbTrigs; k++){
            ...
            gChi2_TrigTypes[k]->SetPoint(graph_index,attenuationPar[i],totalchi2_TrigTypes[k]);
            totalchi2_TrigTypes[k]=0;
        }
    ...
    string outfilename = "file_chi2_results_fit60m.root";
    ...
    gChi2->GetXaxis()->SetTitle("Max. attenuation length (m)");
    ...
    for(int k=0; k<nbTrigs; k++){
        ...
        gChi2_TrigTypes[k]->GetXaxis()->SetTitle("Attenuation length (m)");
    ...
    for(int k=0; k<pmtnb; k++){
        ...
        gChi2_PMTs[k]->GetXaxis()->SetTitle("Attenuation length (m)");
    ...
    */
    ...
    for(int j=0; j<pmtnb; j++){
        for(int k=0; k<nbTrigs; k++){
            ...
            gChi2_indiv[j][k]->GetXaxis()->SetTitle("Max. attenuation length (m)");
...
//            gChi2_indiv[j][k]->SetMarkerColor(j+1);
    ...
    */
    ...
    for(int i=0; i<nbTrigs; i++){
        cIndiv_1[i]->Write();
        sprintf(cName,"fit_60m/chi2_results_trigType_%s.png",strig[i].c_str());
    ...
    */
    ...
    fitPara.clear();
    fstream fout_fit("test_fit_parameters_20190520.txt",ios::out);
    fout_fit<<"True attenuation length is 19.6837 m."<<endl;
    //fit parabolics
    cout<<"\nfit total chi2 "<<endl;
