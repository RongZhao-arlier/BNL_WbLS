...
int main(int argc, char **argv) {
    ...
    const int numberofinputfiles = 150; //148 number of input files
    int nbadruns = 0;
    ...
    const int nbTrigs = 1;
    // trigger types: AllTrig, MultiTrig, HodoOnly, StopMu, ThroughGoingMuons,
    string strig[nbTrigs]={ "hodoTrig"};
	char temprtfilechar[100]; sprintf(temprtfilechar,"Results_v11_hodoTrig_%s.root", argv[1]);
	...
	for(int j=0;j<nbTrigs;j++){
		...
		sprintf(hName,"hDecayZ_%sTrig",strig[j].c_str());
    ...
    TH1F* hCharge1[pmtnb][nbTrigs]; // the total charge in every pmt within a time window of 20ns. This might be useful
	...
	for(int i=0;i<pmtnb;i++){
		for(int j=0;j<nbTrigs;j++){
            ...
            hCharge[i][j] = new TH1F(hName,"",500,0,500);
            hCharge[i][j]->SetXTitle("Number of detected photons");
            hCharge[i][j]->SetYTitle("Counts");
            sprintf(hName,"hCharge_20ns_S%d_%sTrig",i,strig[j].c_str());
            hCharge1[i][j] = new TH1F(hName,"",500,0,500);
            hCharge1[i][j]->SetXTitle("Number of detected photons in 20 ns window after the first photon");
    ...
    for(int ffff=0; ffff<numberofinputfiles; ffff++){ // start loop: reading the root tree files
        ...
        for (int i = 0; i < nEvents; i++) {  
            ...
            float Za = 800.4-25.4; // Point A at the water surface, where track goes through A
            ...
            */
            ...
            if(mctrackcnt!=0){ // this means a decay happend
                for(int imctrackcnt=0;imctrackcnt<mctrackcnt;imctrackcnt++){
                    ...
                    //cout<<"------->stepcnt "<<mctrackstepcnt<<endl;
            ...
            if( (myTriggerEdep[0]>0 || myTriggerEdep[2]>0) && (myTriggerEdep[1]>0 || myTriggerEdep[3]>0) ){
				trigcnts_hodo++;
				...
				if(myTriggerEdep[4]>0. && myTriggerEdep[5]>0.) trigcnts_hodo_H5_H4++;
			}

			bool testTrig = false;			
			//define triggers 
            if( (myTriggerEdep[0]>0 || myTriggerEdep[2]>0) && (myTriggerEdep[1]>0 || myTriggerEdep[3]>0) 
                //&& myTriggerEdep[4]<=0 // using H5 + !H4 for through going muons
              ){
            ...
            for (int imcpmt=0; imcpmt < mc->GetMCPMTCount(); imcpmt++) { // for each pmt
                ...
                double thispmt_charge=0, thispmt_charge_afterdecay=0;
				...
				for(int trigindex=0; trigindex<nbTrigs; trigindex++){
					if(istrigger[trigindex]==true){
						hCharge[pmtID][trigindex]->Fill(thispmt_charge);
						if(firstphotonflag==1){
							hPMTNpeVsTime[pmtID][trigindex]->Fill(thispmt_charge, firstphotontime);							
						}
					}
				}
    ...
    for(int trigindex=0; trigindex<nbTrigs; trigindex++){
      cout<<"Number of "<<strig[trigindex]<<" triggers: "<<trigcnts[trigindex]<<endl;
    }
