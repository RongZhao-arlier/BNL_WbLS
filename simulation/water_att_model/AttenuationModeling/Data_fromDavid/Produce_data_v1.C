...
void Produce_data_v1(){
  ...
  string dataStr[N];
  char temp[200];
  fstream faffff("testlist.txt",ios::out);
  for(int i=0; i<N; i++){
    myMaxAttL[i]=-1.0;
    myWaveAtMaxAttL[i]=0;
    myparameter[i] = 0.06 + 0.002*i;
    myAbsorptionCurve[i] = new TGraph();
    sprintf(temp,"par=%.6f",myparameter[i]);
    myAbsorptionCurve[i]->SetName(temp);

    myAttenuationCurve[i] = new TGraph();
    sprintf(temp,"p=%.6f",myparameter[i]);
    myAttenuationCurve[i]->SetName(temp);

    sprintf(temp,"./third_produced/OPTICS_water_par_%.0f.ratdb",myparameter[i]*100000);
    faffff<<temp<<endl;
  ...
  double ltot;
  double absorption;
  fstream fout2("parameter_maxL_maxLWave1111.txt",ios::out|ios::app);
  ...
  for(int i=0; i<N; i++){
    ...
    fout2<<i<<"\t"<<myparameter[i]<<"\t"<<myMaxAttL[i]/100.0<<"\t"<<myWaveAtMaxAttL[i]<<endl;
    cout<<i<<"\t"<<myparameter[i]<<"\t"<<myMaxAttL[i]/100.0<<"\t"<<myWaveAtMaxAttL[i]<<endl;
  }
  fin2.close();
/*
  //int n=N;
